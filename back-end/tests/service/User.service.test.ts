...
describe('User Service Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockBcryptHash = jest.spyOn(bcrypt, 'hash') as jest.MockedFunction<typeof bcrypt.hash>;
  });
  ...
  test('createUser should create a new user', async () => {
    ...
    mockUserDbCreateUser.mockResolvedValue({
      ...userInput,
    ...
    expect(mockUserDbCreateUser).toHaveBeenCalledWith(expect.objectContaining({
  ...
  test('editUser should update user details successfully', async () => {
    ...
    mockBcryptHash.mockResolvedValue('hashedPassword');
    mockUserDbEditUser.mockResolvedValue({
      ...userInput,
      password: 'hashedPassword',
    });
    ...
    expect(result).toBeDefined();
    expect(mockUserDbEditUser).toHaveBeenCalledWith(1, expect.objectContaining({ 
      username: 'updated_user', 
      password: 'hashedPassword', 
      email: 'updated@example.com', 
      role: 'user' }));
